...
(5) Next, you will proceed the game and program source code directory. Input command "cd puppypi/src/ls" to list all the folders and files of this directory.
...
(1) Start up the device and connect it with VNC remote connection tool according to "[Remote Tool Installation and Container Accessing Method\1.Remote Desktop Tool Installation and Connection]()".
...
According to "[3.3.2 Docker General Commands -> Check Containers]()" to get container ID (the only one). Input command "docker exec -it -u ubuntu -w /home/ubuntu 82df /bin/bash" to enter the container where the functional program is installed. The container ID can be abbreviated as long as it is the unique identification of this container.

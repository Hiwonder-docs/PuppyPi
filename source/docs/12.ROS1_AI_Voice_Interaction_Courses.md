...
(2) Navigate to [05 Appendix\Firmware Flashing Tool](resources_download.md) and open the [PACK_UPDATE_TOOL.exe](resources_download.md) file. Select the CI1302 chip from the list, then click "**Firmware Upgrade**" to begin the process.
...
(3) Select the firmware by locating the `hello hiwonder firmware (Wake word: hello hiwonder).bin` file under the [05 Appendix](resources_download.md) path.
